    ...
    for mid, minfo in metrics.items():
        ...
        for grp in ["PARTNER_TOTAL"] + groups:
            ...
            if pd.isna(slope):
                continue
            score, label = get_trend_score(slope, p, sign)
            group_results.append((score, grp, slope, p, label))
        # 악화(유의) 우선 (점수 낮은 순)
        group_results.sort(key=lambda x: x[0])
        for score, grp, slope, p, label in group_results:
            icon = "🟢" if score > 0 else "🔴"
            warn = " ⚠️" if label == "악화(유의)" else ""
            lines.append(
                f"| {grp} | {fmt(slope,4)} | {fmt(p,4)} | {icon} {label}{warn} |"
            )
        lines += [""]
